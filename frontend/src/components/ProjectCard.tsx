...
export default function ProjectCard({ project, status, onClick, onLaunch, formatUrl }: ProjectCardProps) {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className={`group relative bg-slate-900 border border-slate-800 rounded-xl p-4 hover:border-slate-600 transition-all cursor-pointer shadow-sm hover:shadow-md ${isDragging ? 'z-50 ring-2 ring-indigo-500' : ''}`}
            onClick={onClick}
        >
            {/* Drag Handle */}
            <div
                {...attributes}
                {...listeners}
                className="absolute top-2 left-2 p-1 text-slate-600 hover:text-slate-400 cursor-grab active:cursor-grabbing"
